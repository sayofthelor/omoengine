...
import objects.WiggleEffect.WiggleEffectType;
...
import objects.Note;
import objects.WiggleEffect;
import objects.StaticArrow;
...
class PlayState extends MusicBeatState
{
	...
	private var playerNotes:FlxTypedGroup<Note>;
	private var opponentNotes:FlxTypedGroup<Note>;
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		{
			// FlxG.log.add(i);
			var babyArrow:StaticArrow = new StaticArrow(strumLine.x, strumLine.y, i);
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!playerNotes.members.contains(daNote) && !opponentNotes.members.contains(daNote)) {
					if (daNote.mustPress)
						playerNotes.add(daNote);
					else {
						opponentNotes.add(daNote);
					}
				}

				var strumGrp = playerStrums;
				if (!daNote.mustPress)
					strumGrp = opponentStrums;

				var strumX:Float = strumGrp.members[daNote.noteData].x;

				daNote.x = strumX;

				if (Prefs.middlescroll && !Prefs.hideOpponentNotes) {
					if (!daNote.mustPress)
						daNote.alpha = 0.5;
				}

				if (daNote.isSustainNote) {
					daNote.x += (daNote.width/2)*2;
				}
